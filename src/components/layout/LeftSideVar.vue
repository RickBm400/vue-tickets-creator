<template>
    <aside class="left-side-var__container" :class="rootClass">
        <Menu :model="dashboardPages" class="flex flex-col space-y-12" :pt="{
            list: {
            ...
            <template #item="{ item, props }">
                <RouterLink custom :to="item.route" v-slot="{ href, navigate }">
                    <a v-ripple :href="href" aria-disabled="true" class="menu " v-bind="props.action" @click="navigate"> {{ item.label }} </a>
                </RouterLink>
            </template>
        </Menu>
        <div>
